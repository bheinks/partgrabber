...
<body>
	...
	<p style="font-weight: bold; font-size: 22px; text-align: center;">GPUs</p>
	
	<div style="text-align: center;">
		<form action="gpu.php?build_name=<?=$build_name?>" method="POST">
		...
		<?php
			...
			if($search_field != ""){
				$sql = $sql." AND (
					...
					OR vram LIKE('%".$search_field."%')
					OR name LIKE('%".$search_field."%')
					OR clock_speed LIKE('%".$search_field."%')						
				);";				
			}
